...
|---------|-----------|-----------------|--------|----------|
| **Snapshot promotion to template** | Metadata-only: `POST /v1/snapshots/{id}/promote` inserts a `templates` row | The agent doesn't promote the on-disk blob into the content-addressable image cache. Promoted templates are unusable until that's wired. | 4–6 h | P1 |
